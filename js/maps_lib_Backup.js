...
var MapsLib = {
  ...
  fusionTableId:      "172veabCqjNz6Pybf0iXnArE42qp_acIJBADRQN68", //Point data layer  Original: 1WoxNIvjGQzQAk7B965hwVQOIl04f-Xn09JuTLu03                        
  ...
  //-----custom filters for point data layer
    ...
    var tempWhereClause = [];
    if ( $("#depth1").is(':checked')) tempWhereClause.push("0-20");   //     original dice: tempWhereClause.push("Interdistrict")
    if ( $("#depth2").is(':checked')) tempWhereClause.push("50");  //               tempWhereClause.push("District")
  ...
  submitSearch: function(whereClause, map, location) {
	...
	function windowControl(e, infoWindow, map) {
		...
		var Ref = e.row['Nombre muestra'].value;
		var Dat = e.row['Número de Zona'].value;
		var Col = e.row['Coordenadas'].value;
		// Now build HTML we want in our InfoWindow
		var HTML;
		HTML = "<div class='googft-info-window'>";
		if ( $("#Show0").is(':checked')) HTML += "<strong>Nombre</strong>:" 		+ e.row['Nombre muestra'].value 		+ "<br>";
		if ( $("#Show1").is(':checked')) HTML += "<strong>Pot.Redox mV</strong>:" 	+ e.row['Potencial Redox (mV)'].value 	+ "<br>";
		if ( $("#Show2").is(':checked')) HTML += "<strong>pH</strong>:" 			+ e.row['pH'].value						+ "<br>";
		if ( $("#Show3").is(':checked')) HTML += "<strong>Conduct.</strong>:" 		+ e.row['Conductividad (µS/cm)'].value	+ "<br>";
  ...
  getList: function(whereClause) {
    ...
    var selectColumns = "'Nombre muestra', 'Potencial Redox (mV)', 'pH',  'Conductividad (µS/cm)', Mo, Zr, Sr, U, Rb";
  ...
  displayList: function(json) {
    ...
    if (rows == null) {
      ...
      <table class='table' id ='list_table'>\
        <thead>\
          <tr>"
			if ( $("#Show0").is(':checked')) list_table += "<th>NombreMuestra</th>"
			if ( $("#Show1").is(':checked')) list_table += "<th>Pot.Redox mV</th>"
			if ( $("#Show2").is(':checked')) list_table += "<th>pH</th>"
			if ( $("#Show3").is(':checked')) list_table += "<th>Conduct.</th>"
			list_table += "</tr>\
		</thead>\
		<tbody>";
